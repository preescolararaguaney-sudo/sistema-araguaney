  ...
  return (
    <form action={formAction} className="flex flex-col gap-4 rounded-xl border border-emerald-200 bg-emerald-50 p-5">
      ...
      <h2 className="text-sm font-semibold text-emerald-900">
        {esAlumnoExistente ? "Aprobar y actualizar ficha" : "Aprobar e inscribir"}
      </h2>
      {esAlumnoExistente ? (
        <p className="text-sm text-stone-600">
          Esto actualiza los datos del alumno y guarda representante, padres
          y autorizados a retirar. La matrícula y el plan de pagos no
          cambian, ya existen.
        </p>
      ) : (
      ...
      <button
        type="submit"
        disabled={pending || (!esAlumnoExistente && !anioEscolarId)}
